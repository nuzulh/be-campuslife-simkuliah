...
// netlify/functions/get-jadwal.js
process.env["NODE_TLS_REJECT_UNAUTHORIZED"] = 0;
var main = async (npmReq, passwordReq) => {
  const baseUrl = "https://simkuliah.unsyiah.ac.id/index.php";
  const getCookie = () => new Promise((resolve, reject) => {
    fetch(`${baseUrl}`).then((res) => res.headers.raw()["set-cookie"][0].split("; ")[0]).then((resCookie) => resolve(resCookie)).catch((err) => reject(err));
  });
  const login = (npm, passwd, cookie2) => new Promise((resolve, reject) => {
    fetch(`${baseUrl}/login`, {
      method: "POST",
    ...
    }).then((res) => res.text()).then((resText) => resolve(resText)).catch((err) => reject(err));
  });
  const getJadwal = (cookie2) => new Promise((resolve, reject) => {
    fetch(`https://simkuliah.unsyiah.ac.id/index.php/jadwal_kuliah`, {
      headers: {
        cookie: cookie2
      }
    }).then((res) => res.text()).then((data) => {
      data = data.split("<tbody>").toString().split("</tbody>")[0].toString().split("</thead>")[1];
      let dataMK = [];
  ...
  try {
    const name = resLogin.split('User-Profile-Image">')[1].split("</span>")[0].replace("<span>", "").trim();
...
//# sourceMappingURL=get-jadwal.js.map
